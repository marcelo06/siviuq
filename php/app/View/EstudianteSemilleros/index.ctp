<div class="estudianteSemilleros index">
	<h2><?php echo __('Estudiante Semilleros'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<thead>
	<tr>
			<th><?php echo $this->Paginator->sort('id_estudiante_semillero'); ?></th>
			<th><?php echo $this->Paginator->sort('estudiante_id'); ?></th>
			<th><?php echo $this->Paginator->sort('semillero_id'); ?></th>
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	</thead>
	<tbody>
	<?php foreach ($estudianteSemilleros as $estudianteSemillero): ?>
	<tr>
		<td><?php echo h($estudianteSemillero['EstudianteSemillero']['id_estudiante_semillero']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($estudianteSemillero['Estudiante']['nombre_completo'], array('controller' => 'estudiantes', 'action' => 'view', $estudianteSemillero['Estudiante']['id_estudiante'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($estudianteSemillero['Semillero']['nombre'], array('controller' => 'semilleros', 'action' => 'view', $estudianteSemillero['Semillero']['id_semillero'])); ?>
		</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $estudianteSemillero['EstudianteSemillero']['id_estudiante_semillero'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $estudianteSemillero['EstudianteSemillero']['id_estudiante_semillero'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $estudianteSemillero['EstudianteSemillero']['id_estudiante_semillero']), array(), __('Are you sure you want to delete # %s?', $estudianteSemillero['EstudianteSemillero']['id_estudiante_semillero'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</tbody>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>
	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Estudiante Semillero'), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Estudiantes'), array('controller' => 'estudiantes', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Estudiante'), array('controller' => 'estudiantes', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Semilleros'), array('controller' => 'semilleros', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Semillero'), array('controller' => 'semilleros', 'action' => 'add')); ?> </li>
	</ul>
</div>
